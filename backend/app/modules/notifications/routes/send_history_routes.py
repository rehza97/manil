...
router = APIRouter(prefix="/notifications/send-history",
                   tags=["Email Send History"])
...
async def get_send_history(
    ...
    template_name: Optional[str] = Query(
        None, description="Filter by exact template name"),
    template_prefix: Optional[str] = Query(
        None, description="Filter by template name prefix (e.g. ticket_ for all ticket emails)"),
    ...
    return EmailSendHistoryListResponse(
        items=[EmailSendHistoryResponse.model_validate(
